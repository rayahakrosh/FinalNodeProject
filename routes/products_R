...
router.delete('/:id',(req,res)=>{
    ...
    if(product.myFileName && fs.existsSync(path.join('images',product.myFileName))){ 
...
router.patch('/:id',upload.single('myFile'),(req,res)=>{
    ...
    if(!product){ return res.status(404).json({message:" לא קיים "}) } 
    'isPatch=true'
    const validationError = validateProduct(req, true);
    if (validationError) {
        if (req.file) { fs.unlinkSync(path.join('images', req.file.filename)); }
        return res.status(400).json({ message: validationError });
    }
...
router.patch('/rate/:id', (req, res) => {
    ...
    if (!product) {
        return res.status(404).json({ message: " הפרויקט לא קיים" });
    }
